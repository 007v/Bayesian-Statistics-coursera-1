...
qnorm(c(0.025, 0.975), mean = 10, sd = sqrt(5), lower.tail = TRUE, log.p = FALSE)
```
...
```{r}
print("The probability that the true proportion of females lies in this interval is 0.95")
```
...
xval <- seq(0, 1, .01)
unidis <- dbeta(xval, 1, 1)
strdistu <- dbeta(xval, 500, 500)
df <- data.frame(xval, unidis, strdistu)
t=ggplot(df, aes(xval)) +                       # basic graphical object
       geom_line(aes(y=unidis), colour="red") +   # first layer
       geom_line(aes(y=strdistu), colour="green")+  # second layer
...
```{r}
qbeta(c(0.025, 0.975), 500 + x, 500+n-x)
```
...
```{r}
df$zval <- dbeta(xval,5,200)
t=ggplot(df, aes(xval)) +                       # basic graphical object
       geom_line(aes(y=zval), colour="red") +   # first layer
       xlab("X value")+ylab("PDF")+
       ggtitle("Beta Probability Distribution  Functions")
print(t)
```
...
```{r}
qbeta(c(0.025, 0.975), 5 + x, 200+n-x)
```
...
```{r}
qbeta(c(0.05, 0.95), 1+3868, 1+1132)
```
...
set.seed(1492) # reproducible
xval <- seq(0,10,0.01)
gamma1 <- dgamma(xval,4,1)
gamma2 <- dgamma(xval,12,3)
gamma3 <- dgamma(xval,120,30)
df <- data.frame(xval, gamma1,gamma2,gamma3)
t=ggplot(df, aes(xval)) +                       # basic graphical object
       geom_line(aes(y=gamma1), colour="red") +   # first layer
       geom_line(aes(y=gamma2), colour="green")+  # second layer
       geom_line(aes(y=gamma3), colour="blue")+  # third layer
       xlab("X value")+ylab("PDF")+
       ggtitle("Gamma Probability Distribution  Functions")
print(t)
```
...
library(plyr)
b_post <- length(brfss$fruit_per_day) 
df <- count(brfss$fruit_per_day)
a_post <- sum(df$x*df$freq)
a_post <- 5+a_post
b_post <- 1 + b_post
a_post 
b_post
...
```{r}
qgamma(c(0.05,0.95),shape = a_post ,rate = b_post)
```
...
```{r veg_day}
# type your code for the Exercise here, and Knit
print('No because rate lie in range (1.594, 1.653)') 
```
