...
x <- seq(0.5, 14, length.out=1000)
df <- with(nc, data.frame(x = x, y = dnorm(x, mean(weight), sd(weight))))
p + geom_line(data = df, aes(x = x, y = y), color = "red")
print(p)
print(skewness(nc$weight))
```
...
```{r normal-ci}
# type your code for the Exercise here, and Knit
ggplot(nc) + geom_boxplot(aes(x=habit,y=weight),fill = "grey", color = "blue") 
```
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.5, alternative = "twosided",hypothesis_prior = c(H1=0.3,H2=0.7),beta_prior=c(a=2,b=2))
```
